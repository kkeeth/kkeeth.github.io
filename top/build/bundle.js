...
/* WEBPACK VAR INJECTION */(function(riot) {/* Riot v3.3.1, @license MIT */
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__header_tag__ = __webpack_require__(6);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__footer_tag__ = __webpack_require__(5);
...
riot.tag2('app',
  '<header></header> <div data-is="main"></div> <footer></footer>',
  '',
  '', function(opts) {
});
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__util_mixin_js__ = __webpack_require__(9);
...
this.mode = false;
this.current_page = '';
this.mixin(__WEBPACK_IMPORTED_MODULE_1__util_mixin_js___default.a);

self.on("mount", function () {
	riot_control.on(i.ActionTypes.PAGE_CHANGED, function () {
	...
	riot_control.on(r.ON_MENU_CLOSE, function () {
		self.menu_active = false;
		setTimeout(function () {
			self.menu_sp_overlay = false;
			self.update();
	...
	riot_control.on(r.WINDOW_ON_SCROLL, function () {
		self.loaded = false;
		self.update();
	});

	riot_control.on(r.WINDOW_ON_SCROLL_TOP, function () {
		self.loaded = true;
		self.update();
	});

	riot_control.on(r.ON_MOUSE_AT_TOP, function () {
		self.loaded = true, self.update();
	});

	riot_control.on(r.ON_CATEGORY_SP_OPEN, function () {
		i.data.menu_open = true, self.categoryMode = true, self.menu_sp_overlay = true, self.menu_active = true, self.update();
	});

	_this.update_page = function () {
		_this.current_page = "";
	};

	_this.toggle_menu = function () {
		i.data.menu_open ? (_this.categoryMode && (i.data.menu_open = false), riot_control.trigger(r.ON_MENU_CLOSE)) : (_this.menu_sp_overlay = true, _this.menu_active = true, riot.update(), riot_control.trigger(r.ON_MENU_SELECTED));
	};
});
});
